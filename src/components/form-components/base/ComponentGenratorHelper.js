...
export function classBuilder(el, confGlobal, className){
  ...
  if(clazzes){
    if(typeof clazzes === 'string'){
      return `${className ? className : 'class'}='${clazzes}'`
    }else if(Array.isArray(clazzes)){
      return `${className ? className : 'class'}='${clazzes.join(' ')}'`
    }
  }
...
// span不为24的用el-col包裹
export function colWrapper(scheme, str, someSpanIsNot24, forceWrap) {
  if (forceWrap || someSpanIsNot24 || scheme.__config__.span !== 24) {
